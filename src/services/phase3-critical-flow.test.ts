...
describe("PHASE 3 — Critical Flow: contract & integration", () => {
  ...
  describe("Registration → Payment root aggregate", () => {
    it("registerEvent builds POST /events/{id}/register and unwraps { success, data }", async () => {
      ...
      const order: Partial<Order> = { uuid: "ord-uuid", nomor_order: "ORD-1", status_registrasi: "registered" as const };
      ...
      const res = await registerEvent(42);
      expect(postSpy).toHaveBeenCalledWith("/events/42/register", undefined, expect.anything());
    ...
    it("duplicate registration surfaces as ApiError 409", async () => {
      ...
      await expect(registerEvent(42)).rejects.toMatchObject({ status: 409 });
      try {
        await registerEvent(42);
      } catch (e) {
  ...
  describe("Payment Verification Queue", () => {
    it("fetchVerificationQueue encodes params and drops null/empty", async () => {
      ...
      const spy = vi.spyOn(apiClient, "get").mockResolvedValue({
        data: envelope({ data: [], current_page: 1, last_page: 1, per_page: 15, total: 0 } as VerificationQueueResponse),
      });
    ...
    it("query key is parameter-aware (different params → different keys)", () => {
      const k1 = verificationQueueQuery({ status: "waiting_verification", page: 1, per_page: 15 } as VerificationQueueParams).queryKey;
      const k2 = verificationQueueQuery({ status: "paid", page: 1, per_page: 15 } as VerificationQueueParams).queryKey;
      const k3 = verificationQueueQuery({ status: "waiting_verification", page: 2, per_page: 15 } as VerificationQueueParams).queryKey;
    ...
    it("verifyPayment uses PUT /payments/{uuid}/verify and returns changed flag", async () => {
      ...
      const spy = vi.spyOn(apiClient, "put").mockResolvedValue({ data: { success: true, data: payload } });
    ...
    it("verifyPayment propagates ApiError on 403/404/422", async () => {
      ...
      for (const status of [403, 404, 422]) {
        vi.spyOn(apiClient, "put").mockRejectedValueOnce(axiosError(status, errorEnvelope("forbidden")));
    ...
    it("fetchPaymentDetail unwraps envelope", async () => {
      ...
      const data = { payment: { uuid: "p1" } as never, outstanding: { total: 100, paid: 100, outstanding: 0, payment_status: "paid" } };
  ...
  describe("Ticket Issuance", () => {
    ...
    it("reissueTicket posts note and unwraps ticket", async () => {
      ...
      expect(spy).toHaveBeenCalledWith("/tickets/t1/reissue", { note: "reprint" }, expect.anything());
    });
    ...
    it("revokeTicket deletes with note", async () => {
      ...
      const spy = vi.spyOn(apiClient, "delete").mockResolvedValue({ data: { success: true, data: { ticket } } });
  ...
  describe("Check-in", () => {
    it("checkIn posts to /checkin and returns CheckInResult", async () => {
      ...
      const result: CheckInResult = {
        ticket: { id: 1, uuid: "t1", nomor_ticket: "T-1", status: "checked_in" as const, used_at: new Date().toISOString() },
        attendance: { id: 10, id_event: 1, id_tanggal: 1, id_anggota: 1, id_ticket: 1, gate: "A", scanned_at: new Date().toISOString(), scanned_by: 1 },
        participant: { id: 1, id_anggota: "MZT001", name: "A" },
        event: { id_event: 1, event_name: "E" },
      } as CheckInResult;
      const spy = vi.spyOn(apiClient, "post").mockResolvedValue({ data: { success: true, data: result } });
    ...
    it("duplicate checkIn surfaces 409 with first scan info", async () => {
      const { checkIn } = await import("@/services/mzt-api");
      const dup = { success: false, message: "already checked", data: { first_scanned_at: new Date().toISOString(), first_scanned_by: 1 } };
      vi.spyOn(apiClient, "post").mockRejectedValue(axiosError(409, dup));
      await expect(checkIn({ ticket_uuid: "t1", id_tanggal: 1 })).rejects.toMatchObject({ status: 409 });
    });

    it("invalid ticket uuid → 404, expired ticket → 409", async () => {
      const { checkIn } = await import("@/services/mzt-api");
      vi.spyOn(apiClient, "post").mockRejectedValueOnce(axiosError(404, errorEnvelope("not found")));
      await expect(checkIn({ ticket_uuid: "bad", id_tanggal: 1 })).rejects.toMatchObject({ status: 404 });
  ...
  describe("Attendance & Operations (read-only evidence)", () => {
    ...
    it("fetchParticipants paginates and filters by tgl/gate/q", async () => {
      ...
      const resp: ParticipantListResponse = { rows: [], meta: { total: 0, page: 1, per_page: 20, last_page: 0, filter: {} } };
    ...
    it("fetchAttendanceSummary returns present/legacy/total", async () => {
      ...
      const summary: AttendanceSummary = { event_id: 1, tanggal_id: 1, present: 5, legacy_count: 2, total: 7, per_tanggal: [] };
    ...
    it("fetchGateMonitoring groups by gate", async () => {
      ...
      const resp: GateMonitoringResponse = { event_id: 1, tanggal_id: 1, rows: [{ gate: "A", present: 3, legacy: 0, total: 3 }], breakdown_per_gate: {} };
  ...
  describe("Audit Timeline — final evidence", () => {
    it("fetchAuditTimeline encodes all filters and drops null/empty", async () => {
      ...
      await fetchAuditTimeline({ event_id: null, date_from: null, q: "", page: null } as AuditTimelineParams);
    ...
    it("query key is parameter-aware for all filter combos", () => {
      const p1: AuditTimelineParams = { event_id: 1, entity_type: "payment", page: 1, per_page: 20 };
      const p2: AuditTimelineParams = { event_id: 1, entity_type: "ticket", page: 1, per_page: 20 };
      const p3: AuditTimelineParams = { event_id: 2, entity_type: "payment", page: 1, per_page: 20 };
      const p4: AuditTimelineParams = { event_id: 1, entity_type: "payment", page: 2, per_page: 20 };
    ...
    it("auditTimelineQuery uses retry 0 and does not refetch without param change", async () => {
      const q1 = auditTimelineQuery({ event_id: 1, page: 1, per_page: 20 }) as unknown as { retry: number; queryKey: unknown };
    ...
    it("audit rows contain required fields", async () => {
      ...
      vi.spyOn(apiClient, "get").mockResolvedValue({ data: envelope({ rows, total: 1 } as AuditTimelineResponse) });
      const res = await fetchAuditTimeline({ event_id: 1, page: 1, per_page: 20 });
      expect(res.rows[0]).toMatchObject({ actor: "finance", entity: "payment", old_status: "waiting_verification" });
    ...
    it("server pagination: total vs rows length", async () => {
      ...
      vi.spyOn(apiClient, "get").mockResolvedValue({
        data: envelope({ rows: Array.from({ length: 20 }, (_, i) => ({ actor: "a", action: "v", entity: "payment" as const, entity_id: i, timestamp: new Date().toISOString() })), total: 100 } as AuditTimelineResponse),
      });
  ...
  describe("Authorization & deterministic errors", () => {
    it("FINANCE_ROLES guard blocks non-finance (checked via requireRoles)", async () => {
      ...
      await expect(requireRoles(client as never, FINANCE_ROLES)).rejects.toSatisfy((e: unknown) => isRedirect(e as never));
    });
  ...
  describe("E2E sequence: Registration → Payment verify → Ticket → Check-in → Attendance → Audit", () => {
    it("full happy path mocked via apiClient", async () => {
      const { registerEvent, verifyPayment, fetchMyTicket, checkIn, fetchAttendanceSummary, fetchAuditTimeline } = await import("@/services/mzt-api");

      // 1) register
      vi.spyOn(apiClient, "post").mockResolvedValueOnce({ data: { success: true, data: { uuid: "ord1", status_registrasi: "registered" } } });
      const reg = await registerEvent(1);
      expect((reg as { data: { uuid: string } }).data.uuid).toBe("ord1");

      // 2) verify payment paid
      vi.spyOn(apiClient, "put").mockResolvedValueOnce({ data: { success: true, data: { payment: { uuid: "pay1", status: "paid" }, changed: true } } });
      const ver = await verifyPayment("pay1", { status: "paid" });
      expect(ver.success).toBe(true);

      // 3) ticket issued
      vi.spyOn(apiClient, "get").mockResolvedValueOnce({ data: envelope({ ticket: { uuid: "t1", status: "issued" } }) });
      const ticket = await fetchMyTicket("ord1");
      expect(ticket.status).toBe("issued");

      // 4) check-in
      const chkRes: CheckInResult = {
        ticket: { id: 1, uuid: "t1", nomor_ticket: "T1", status: "checked_in", used_at: new Date().toISOString() },
        attendance: { id: 1, id_event: 1, id_tanggal: 1, id_anggota: 1, id_ticket: 1, gate: "A", scanned_at: new Date().toISOString(), scanned_by: 1 },
        participant: { id: 1, id_anggota: "MZT001", name: "A" },
        event: { id_event: 1, event_name: "E" },
      } as CheckInResult;
      ...
      const summary: AttendanceSummary = { event_id: 1, tanggal_id: 1, present: 1, legacy_count: 0, total: 1, per_tanggal: [] };
      ...
      const audit: AuditTimelineResponse = {
        rows: [{ actor: "prisensi", action: "checkin", entity: "checkin", entity_id: 1, old_status: "issued", new_status: "checked_in", timestamp: new Date().toISOString(), note: null }],
        total: 1,
      };
      vi.spyOn(apiClient, "get").mockResolvedValueOnce({ data: envelope(audit) });
      const aud = await fetchAuditTimeline({ event_id: 1, entity_type: "checkin", page: 1, per_page: 20 });
